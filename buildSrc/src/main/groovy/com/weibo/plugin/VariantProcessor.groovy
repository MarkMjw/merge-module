...
class VariantProcessor {
    ...
    private void processArtifacts(Task bundleTask) {
        for (DefaultResolvedArtifact artifact in mResolvedArtifacts) {
            if (DependencyPlugin.ARTIFACT_TYPE_JAR == artifact.type) {
                addJarFile(artifact.file)
            } else if (DependencyPlugin.ARTIFACT_TYPE_AAR == artifact.type) {
                ...
                archiveLibrary.getExplodedRootDir().deleteDir()
                def zipFolder = archiveLibrary.getRootFolder()
                zipFolder.mkdirs()
                if (buildDependencies.size() == 0) {
                    mProject.copy {
                        Logger.i("explode $artifact.name")
        ...
        if (mVariant.getBuildType().isMinifyEnabled()) {
            ...
            for (archiveLibrary in mAndroidArchiveLibraries) {
                ...
                for (File file : thirdProguardFiles) {
                    if (file.exists()) {
                        Logger.i('add proguard file: ' + file.absolutePath)
    ...
    private void processResourcesAndR() {
        ...
        resourceGenTask.doFirst {
            for (archiveLibrary in mAndroidArchiveLibraries) {
                Logger.i("Merge resource，Library res：${archiveLibrary.resFolder}")
    ...
    private def processRFile() {
        ...
        bundleAar.doFirst {
            Logger.i("Assemble final aar, from:$outputDir")
            mProject.copy {
        ...
        bundleAar.doLast {
            Logger.i("Assemble final aar, target:$outputFile")
        }
    ...
    private Task createRFileTask(def destFolder) {
        ...
        task.doLast {
            mAndroidArchiveLibraries.each {
                Logger.i("Generate R File, Library:${it.name}")
    ...
    private Task createRClassTask(def sourceDir, def destinationDir) {
        ...
        task.doFirst {
            Logger.i("Compile R.class, Dir:${sourceDir.path}")
        }
        return task
    }
    ...
    private Task createRJarTask(def fromDir, def desFile) {
        ...
        task.doFirst {
            Logger.i("Generate R.jar, Dir：$fromDir")
        }
        return task
    }
    ...
    private ConfigurableFileCollection getClassPathDirFiles() {
        ...
        if (gradleVersion != null && gradleVersion.contains("3.2")) { // Versions 3.2.x
            classpath = mProject.files("${mProject.buildDir.path}/intermediates/javac/${mVariant.name}/compile${mVariant.name.capitalize()}JavaWithJavac/classes")
