...
class DependencyPlugin implements Plugin<Project> {
    ...
    private void checkAndroidPlugin() {
        if (!project.plugins.hasPlugin('com.android.library')) {
            throw new ProjectConfigurationException('fat-aar-plugin must be applied in project that has android library plugin!', null)
        }
    }
    ...
    private void resolveArtifacts() {
        ...
        embedConf.resolvedConfiguration.resolvedArtifacts.each { artifact ->
            ...
            if (ARTIFACT_TYPE_AAR == artifact.type || ARTIFACT_TYPE_JAR == artifact.type) {
                Logger.i('[embedded detected][' + artifact.type + ']' + artifact.moduleVersion.id)
            } else {
